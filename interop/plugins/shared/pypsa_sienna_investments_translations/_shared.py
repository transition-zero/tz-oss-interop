...
FOM_CHARGE_COL = "_fom_charge_per_mw_year"

FOM_CHARGE_DERIVATION = (
    "the yearly fixed charge, from the extensions sidecar where a PLEXOS model stated one and "
    "from fom_cost otherwise, as the fixed term of the operation cost"
)


def yearly_fixed_charge(fom_cost_col: str) -> pl.Expr:
    """What one MW costs to keep for a year.

    A network this translator wrote carries the charge in the sidecar, because PyPSA reads
