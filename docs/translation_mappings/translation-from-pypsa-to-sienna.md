...
A component whose `effective_p_nom` is 0 is left out for the same reason, whichever column the 0 came from. A solve that builds none of an extendable component writes `p_nom_opt` 0, so the plan refused that build and no capacity stands there for an operations model to dispatch. A non-extendable component stating `p_nom` 0 reaches the same place. Both are recorded as skipped, and the event names the column the rating was read from.
