...
| --- | --- | --- |
| `Constraint`. A weighted sum over named objects, held above or below a right-hand side. | SiennaSchemas states no generic constraint. A search of `Core/`, `Operations/`, `Investments/`, `TimeSeries/` and `Dynamics/` finds no coefficient field on any type. The seven `Investments/Requirements/` types are fixed policy forms, and each one holds the whole portfolio or a named member list, not a weighted sum. | CAISO 373, AEMO 186, SEM 18 |
